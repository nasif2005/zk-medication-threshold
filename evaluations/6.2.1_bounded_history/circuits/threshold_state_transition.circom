 ...
 */
template ThresholdStateTransition(k, nBits, sumBits) {
    ...
    // Private Poseidon Merkle authentication path of fixed depth two.
    pathIndex[0] * (pathIndex[0] - 1) === 0;
    pathIndex[1] * (pathIndex[1] - 1) === 0;
    signal left0;
    signal right0;
    signal node1;
    signal left1;
    signal right1;
    left0 <== commitPrev + pathIndex[0] * (pathElements[0] - commitPrev);
    right0 <== pathElements[0] + pathIndex[0] * (commitPrev - pathElements[0]);
    component merkleHash0 = Poseidon(2);
    merkleHash0.inputs[0] <== left0;
    merkleHash0.inputs[1] <== right0;
    node1 <== merkleHash0.out;
    left1 <== node1 + pathIndex[1] * (pathElements[1] - node1);
    right1 <== pathElements[1] + pathIndex[1] * (node1 - pathElements[1]);
    component merkleHash1 = Poseidon(2);
    merkleHash1.inputs[0] <== left1;
    merkleHash1.inputs[1] <== right1;
    merkleHash1.out === rootPub;
}
